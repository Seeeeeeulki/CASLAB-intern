...
static int read_counter(struct perf_evsel *counter)
{
	...
	for (thread = 0; thread < nthreads; thread++) {
		for (cpu = 0; cpu < ncpus; cpu++) {
			...
			if (perf_evsel__read(counter, cpu, thread, count)) {  //this term_seul
...
static void process_interval(void)
{
    ...
    evlist__for_each_entry(evsel_list, counter) {
		...
		temp = print_counter_aggr(counter);
        fprintf(stderr, "%ld,", temp);
   }
}
		...
		while (!done) {
			...
			if (interval){
	            ...
	            evlist__for_each_entry(evsel_list, counter) {
                    read_counter(counter);
        	        perf_stat_process_counter(&stat_config, counter);
	        		sprintf(buff, "%ld", print_counter_aggr(counter));
                    if((retval = write(fd, buff, strlen(buff))) < 0)
                        fprintf(stderr, "write error\n");
                    if((retval = read(fd, s, strlen(buff))) < 0)
		     ...
		     char *prefix, u64 run, u64 ena, double noise)
{
	...
	if (metric_only)
	    ...
	    fprintf(stderr, "%ld,", (long)uval);
        //find_print_stderr_seul
    if (3==4){
    	out.print_metric = pm;
	    out.new_line = nl;
    	out.ctx = &os;
 ...
 */
static long print_counter_aggr(struct perf_evsel *counter){
